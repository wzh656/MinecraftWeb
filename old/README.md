# 我的世界（仿）<img src="https://wzh.glitch.me/Minecraft/img/icon_small.png" />
...
![修改日期-2021.8.18](https://img.shields.io/badge/修改日期-2021.8.18-blueviolet)
...
2. 网页版运行：可以直接访问 <https://minecraftweb.netlify.app/home.html> （较快）
    ...
    > 3. 竖屏运行时会有一个提示使用横屏或者下载，若浏览器支持横屏，可以将手机横屏使用；
        也可以点击关闭，直接竖屏运行
...
在原版风格基础上**创新性地**增加游戏**真实性**，但拒绝完全复制（体现在“*仿*”字），
同时处理好创新与真实的关系，既在创新之中体现真实，又在真实之中发展创新，故名之***两个原则***  
《我的世界》风格的界面仅是一个*载体*和*躯壳*，却非真正的*灵魂*所在。

### 三个战略
目前，本项目仍处在初级发展阶段，bug和性能方面都不是很好。对此，根据*真实性原则*，需要坚持*三个战略*（即*改革优化创新战略*）：
1. ***改革***：泛指修bug与代码体系修改；
...
规定：世界中的每1px为1cm，每100px为1m，每个*普通方块*为边长1m=100cm=100px的立方体。
> 注：*deskgood*~~为某初中同桌的外号~~，作为游戏主角名，莫要在意名称![](https://img.shields.io/github/search/wzh656/MinecraftWeb/deskgood)
...
| `F2`						| 截图												|
| `F3`						| 打开/关闭dat.gui									|
| `shift`+`F3`				| 关闭/开启调试（将自动刷新）						|
| `F4`						| 开始/停止录屏								|

> ### 手机版（触屏版）

1. 移动：左边的大圆
    摇杆操作，`touch`移动控制行走，拖拽超过`100px`疾跑
2. 跳跃：右边的小圆
    可以与移动同时操作。  
    按下(`touch-start`)之后，如果脚下有方块并且距离上一次跳跃超过约1s（需有休息时长），
    则将沿着y轴正半轴方向以约5m/s的速度跳起来
3. 滑动屏幕界面：转动角色视角
4. 短按屏幕界面：`touch`<1s
...
由于项目长期处于调试状态，调试工具可能默认开启，调试工具可能影响游戏性能，若无需使用请按`Shift`+`F3`关闭
...
## （四）功能 & 计划
### 特色创新功能(Feature)
1. [x] 通过 `F=ma`, `v=at`, `s=vt` 等经典物理公式计算玩家运动加速度、速度及位移
2. [x] 考虑空气阻力 `f = 1/2*CρSV²` 对玩家的影响
3. [x] 考虑宇宙无规则随机扰动产生的蝴蝶效应对一切参数数据的影响
4. [x] 【特性】`XYZ`轴均无限、区块无限的世界
5. [x] 【特性】爬树功能
6. [x] 根据玩家设置的*经纬度*和*开始日期时间*计算日出日落时间和天空颜色、环境光颜色
7. [x] 具有降雨量控制天气系统
8. [x] 根据种子(`seed`)用柏林噪声(`simplexNoise`)随机生成的具有区块类型、地形、土壤层厚度、树高、树叶高、降雨量等完善属性的区块系统
9. [x] 独立于现实且可更改流速、可进行`setTimeout`, `setInterval`, `clearTimeout`, `clearInterval`操作的时间系统

> 注：此处的*特性*指偶然的产生功能

### 计划(Plans)
1. [ ] 【doing】【创新】玩家状态界面：健康、饥饿、口渴、精神、疲惫、经验等
2. [ ] 【doing】【创新】工具物品合成界面
3. [ ] 【改革】使用`Babylon.js`和js模块化重写
4. [ ] 【优化】尝试使用多线程工作（除涉及`scene`等主线程对象以外）
5. [ ] 【改革】使游戏更加科学：加入物理引擎，方块重力及电磁力
6. [ ] 【创新】联机：尝试使用内网联机(`electron`)和服务器外网联机
7. [ ] 更多……【未解密】天机不可泄露
...
## （五）感谢(Thanks)
+ 感谢[*deskgood*](https://github.com/deskgood)对本项目的支持并提供了一些想法和建议
...
## （八）更多(More)
更多*特性*请自行[运行](#运行)尝试或*在[代码](https://github.com/wzh656/MinecraftWeb/blob/master/main.html)中领悟*。
