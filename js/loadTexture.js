...
$("#progress progress")[0].max = texture_load.keys.length; //进度条最大进度（贴图加载）
texture_load.id = setInterval(async function(){
	...
	for (let j in block.block.face){
		...
		j );
		console.log(texture_load.i, block, j)
	}
